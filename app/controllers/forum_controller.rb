class ForumController < ApplicationController
  ...
  def destroy
    ...
    flash[:notice] = "El post #{@post.subject} se ha sido eliminado."
    redirect_to :action => 'index'
  end
  ...
  def show
    ...
    #@posts.delete(@post.root_id)
    #@responses = ForumPost.find_by_root_id(@post.id)
  ...
  def index
    @posts = ForumPost.order('root_id desc , created_at').paginate(:page => params[:page],
                                                                    :per_page => 10)
    @page_title = 'Foro'
  end
